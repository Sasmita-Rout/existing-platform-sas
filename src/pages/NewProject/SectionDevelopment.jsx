...
export default function SectionDevelopment({
  ...
  const developmentInputs = [
    ...
    { key: "server_side_backend_frameworks", labels: "Select Server-Side & Back-End Frameworks" },
    ...
    { key: "version_control_system_vcs", labels: "Select Version Control System (VCS)" },
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        console.log('SectionDevelopment - Starting fetch...');
        const response = await fetch(
          `https://intranet.accionlabs.com/pmoreporting/platform_data/column_dropdown`
        ).then((r) => r.json());

        console.log('SectionDevelopment - API Response:', response);

        // Handle both response formats: {values: {...}} or direct object
        const result = response.values || response;
        console.log('SectionDevelopment - Normalized result:', result);

        const dataObj = {};
        developmentInputs.forEach((input) => {
          const apiValues = result[input.key];
          dataObj[input.key] = apiValues && apiValues.length > 0 ? apiValues : [];
          console.log(`SectionDevelopment - ${input.key}:`, dataObj[input.key]?.length, 'items');
        });
        console.log('SectionDevelopment - Fetched options:', dataObj);
        setOptions(dataObj);
      } catch (err) {
        console.error("SectionDevelopment - Error fetching dropdowns:", err);
        setOptions({});
      } finally {
        setLoading(false);
      }
  ...
  useEffect(() => {
    if (viewProject && row) {
      console.log('SectionDevelopment - viewProject and row detected');
      const initial = {};
      developmentInputs.forEach(({ key }) => {
        const value = row[key];
        console.log(`SectionDevelopment - row[${key}]:`, value, 'type:', typeof value);
        if (Array.isArray(value)) {
          initial[key] = value;
        } else if (value) {
        ...
        console.log(`SectionDevelopment - initial[${key}]:`, initial[key]);
      });
      console.log('SectionDevelopment - Setting viewValues:', initial);
